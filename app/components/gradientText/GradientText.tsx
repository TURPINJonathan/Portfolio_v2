...
export default function GradientText({
  ...
  const containerClasses = [
    ...
    'justify-center',
    'overflow-hidden',
    showBorder ? 'rounded-[1.25rem] backdrop-blur transition-shadow duration-500 py-1 px-2 font-medium' : '',
    pauseOnHover ? 'cursor-pointer' : 'cursor-default',
    !isInline ? 'mx-auto' : '',
    className,
  ]
    .filter(Boolean)
  ...
  return (
    <Container className={containerClasses} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      {showBorder && (
        <motion.div
